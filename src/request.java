...
/* konstruktor */
request(int htver,int met,String url,ArrayList <String> head,DataInputStream in,DataOutputStream out) throws MalformedURLException
{
 ...
 */
 ...
 headers:for(i=0;i<cachedsize;i++)
 {
    ...
    if(s1.equals("pragma"))
           {
	       reload=true;
	       if (remove_pragma_no_cache) { headers.remove(i--);cachedsize--;}
	       continue;
           }
    if(s1.equals("cache-control"))
    {
	 ...
	 if(s2.equals("no-cache"))
           reload=true;
       continue;
    }
    if(s1.equals("host")) { hasHost=true; /* hosth=s2;*/ continue;}
    ...
    if(s1.equals("connection") || s1.equals("proxy-connection")) {
	                                if(s2.length()==5)		// length of 'Close' string 
	                                	keepalive=false; 
	                                else
	                                	keepalive=true;
	                                headers.remove(i--);
    ...
    if(s1.equals("via")) {
                                        headers.set(i, "Via: "+s2+","+via_header);
                                        hasVia=true;
                                        continue;
                                      }
 ...
 if(httpv<10) headers.add("Accept: */*");
 if(proxyvia && !hasVia) headers.add("Via:"+via_header);

 /* - F O R W A R D E R - */
 if(url.charAt(0)=='/')
 {
   throw new MalformedURLException("Forwarding is currently disabled due to possible infinite loop bug");
   
   /* NOTE: Forwarder engine disabled due to security problem
 ...
 try
 {
  ...
  if(zav>0)
  {
   ...
   { throw new MalformedURLException("Incorrect auth syntax in URL");} // So no need to check index out of bounds
 } 
 else 
 {
   protocol="https";
...
final private int get_and_update_maxforwards()
{
  ...
  for(int i=0;i<cachedsize;i++)
  {
    ...
    if(s1.equals("max-forwards")) {
                ...
                if(maxforwards>0) headers.set(i, "Max-Forwards: "+(maxforwards-1));
                return maxforwards;
    }
 }
 return -1; // NONE
...
final public void run_request(WebConnection wc) throws IOException
{
 ...
 if(retry)
 {
	 ...
	 if(mgr.loglevel>3) System.out.println(Thread.currentThread().getName()+" No response from server "+getURL()+", will retry.");
...
/**
 *  reads headers from server and parses them.
 *  multiline headers are merged.
 */
final public void read_headers(String firstline,WebConnection wc) throws IOException
{
 ...
 int expi=-1;
 int cci=-1;
 line=firstline;	/* needed for trace_reply purposes */
 httpreq.read_http_headers(in, headers);
 /* process headers */
      ...
      )
      {
    	  ...
    	  if(s2.indexOf("expires")!=-1 || s2.indexOf("max-age")!=-1)
    	  {
    		  ...
    		  for(int k=mgr.allow_cookies_to.length-1;k>=0;k--)
    			  if(hostname.endsWith(mgr.allow_cookies_to[k])) 
    			  {
    				  allowed=true;
    				  break;
    			  }
 ...
 if(keepalive==false)
 {
     headers.add("Proxy-Connection: close");
 ...
 if(exp!=0) /* Check Expire: against Date: header */
 {
      ...
      long now=0;
      for(String h: headers.toArray(new String[0]))
      {
       String s1,s2;
       int j;
       j=h.indexOf(':',0);
       if(j==-1) continue;
       s1=h.substring(0,j).toLowerCase();
       if(s1.equals("date")) {
                                        try
                                        {
                                           s2=h.substring(j+1).trim();
      ...
      {  
          ...
          reload=true;
          headers.remove(expi); /* Drop EXPIRE: header !*/
          if(pnci>expi) pnci--;
          if(cci>expi) cci--;
	...
	{ 
	    ...
	    throw new IOException("Aborted transfer time timed out");
			     ...
			     if(CheckQuickAbort(read)) 
			       { ctsize=read;file.close();wc.close();throw e;}
			     clientdead=true;
...
final public void make_headers(int rc,String ctype,String enc,String loc,int sz,long lm,long exp,String et)
{
 ...
 if(sz>=0) {
	 headers.add("Content-Length: "+sz);
 }
 else
     if(rc!=304) keepalive=false;
 if(lm!=0) headers.add("Last-Modified: "+printDate(new Date(lm)));

 if(et!=null) headers.add("ETag: "+et);
 /*
 ...
 */
 if(exp>0) headers.add("Expires: "+printDate(new Date(exp)));
 if(loc!=null) headers.add("Location: "+loc);
 ...
 headers.add("Date: "+printDate(d));
 headers.add("Server: "+scache.VERSION);
}
