...
public final static String CACHEVER="0.94";
...
final public static void main(String argv[])
{
 ...
 "See the GNU General Public License version 2 for copying conditions.\n");

 /* print quote to stderr */
 System.err.println(quotes[(int)(Math.random()/(1.0f/quotes.length))]);
 System.err.println("");
 ...
 for(int i=0;i<argv.length;i++)
 {
	...
	filesystemchecked=false;
	 ...
	 mgr.swap_level1_dirs=mgr.swap_level2_dirs=z;
	 i++;
       }
    ...
    if(argv[i].equals("-nofail"))  {
	             ...
	             mgr.pass=null;
			     mgr.regex_fail=null;
			     mgr.fail_filename=null;
			     mgr.pass_filename=null;
			     mgr.regex_fail_filename=null;
	   System.err.println("[INFO] URL filter turned off.");
                                  }
      else
    if(argv[i].equals("-cookies"))  {
	               mgr.allow_cookies_to=null;
				   mgr.cookie_filename=null;
    ...
    if(argv[i].equals("-fastrefresh"))  {
	             mgr.reload_age=0;
			     mgr.min_age=0;
			     mgr.max_age=60*60*1000L*24*3;  // max age 3 days
			     mgr.redir_age=0;
			     mgr.expire_age=0;
			     mgr.refresh=null;
			     mgr.lmfactor=0.2f;
  ...
  while(ACT>0)
  {
   ...
   if( (ACT & ACT_REPAIR)>0)
    {
     ...
     repair.repairDir(mgr.cache_dir,true);
   ...
   if( (ACT & ACT_NOCACHE)>0)
    {
	   mgr.cacheonly=false;
	   mgr.nocache=mgr.addRegexpToArray("*",null,true);
	   ACT-=ACT_NOCACHE;
   ...
   {
      try {
       mgr.http_proxy=InetAddress.getLocalHost();
	   mgr.reload_age=Long.MAX_VALUE;
	   mgr.min_age=Long.MAX_VALUE;
	   mgr.max_age=Long.MAX_VALUE;
	   mgr.redir_age=Long.MAX_VALUE;
	   mgr.expire_age=Long.MAX_VALUE;
	   mgr.refresh=null;
	   mgr.lmfactor=99999;
	   mgr.no_proxy=null;
       mgr.http_proxy_port=1; // I HOPE THAT THIS PORT NUMBER IS UNUSED
       mgr.https_proxy=mgr.http_proxy;
	   mgr.https_proxy_port=mgr.http_proxy_port;
