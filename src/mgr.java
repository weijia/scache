...
public static int http_proxy_port;    // parent proxy port
...
public static int ftp_proxy_port;    // parent proxy port
...
public   static  java.util.regex.Pattern regex_fail[];
public   static  java.util.regex.Pattern regex_pass[];
...
private  static HashMap <String, cachedir> dircache;
...
mgr()
{
 ...
 cacheobject.c_refresh = new AtomicInteger();
 cacheobject.b_hit = new AtomicInteger();
 cacheobject.b_miss = new AtomicInteger();
 ...
 ourport=3128; /* port number used by squid */
 ...
 cacheobject.save_noncacheable=false;
 case_sensitive=true;
 ...
 while ( (line = dis.readLine()) != null)
 {
      ...
      try
      {
         ...
         if(token.equalsIgnoreCase("defaultname")) {cacheobject.defaultname=st.nextToken();
          ...
          if(token.equalsIgnoreCase("save_noncacheable")) {
              									   cacheobject.save_noncacheable=getYesNo(st.nextToken());
              					continue;
              									   }
...
final public void process_request(request req) throws MalformedURLException, IOException
{
 ...
 /* Mascot of Commodore C64 is Captain Hook because C64 created piracy */
     ...
     if(!uireq)
     {
    	 dmachine:for(int i=dmachine_mask.length-1;i>=0;i--)
    	 {
    		 if(dmachine_mask[i].matches(requested))
    		 {
    			 ...
    			 synchronized(this)
    			 {
    				 HashMap <String, String> dmq=ui.load_dm_queue();
...
final public void run()
{
 ...
 int i=0;
 Thread.currentThread().setName("background .cacheinfo saver");
 while(true)
 {
   try
   {
 ...
 if(checkFlag(immediate_shutdownflag,clear_flags_on_shutdown))
 {
	synchronized ( mgr.class )
	{
		...
		for(cachedir d: dircache.values())
		{
			d.save();
...
final private synchronized void save()
{
 ...
 if(saved>0) // write hit stats
 {
   ...
   sb.append(
   (int)(
   100.0f*(cacheobject.c_hit.get()+cacheobject.c_block.get())  /
   ((float)cacheobject.c_hit.get()+cacheobject.c_block.get()
                +cacheobject.c_miss.get()+cacheobject.c_refresh.get())
        )   );
   sb.append("% REQS: ");
   sb.append(cacheobject.c_hit.get());
   sb.append(" Hit, ");
   sb.append(cacheobject.c_refresh.get());
   sb.append(" Refresh, ");
   sb.append(cacheobject.c_miss.get());
   sb.append(" Miss, ");
   sb.append(cacheobject.c_block.get());
   sb.append(" Block.");
   ...
   // clear the stats
   cacheobject.c_hit.set(0);
   cacheobject.c_miss.set(0);
   cacheobject.c_refresh.set(0);
   cacheobject.c_block.set(0);
 }
 if(cacheobject.b_miss.get()+cacheobject.b_hit.get()>0)
 {
   StringBuilder sb=new StringBuilder(100);
   sb.append(new Date());
   sb.append(" - ");
   sb.append(
   (int)(
   100.0f*cacheobject.b_hit.get()/(cacheobject.b_miss.floatValue()+cacheobject.b_hit.floatValue())
   )         );
   sb.append("% BYTES: ");
   sb.append(cacheobject.b_miss.get());
   sb.append(" B in, ");
   sb.append(cacheobject.b_miss.get()+cacheobject.b_hit.get());
   sb.append(" B out.");
   if(loglevel>2) System.out.println(sb);
   if(dos!=null)
     try
   ...
   // clear the stats
   cacheobject.b_hit.set(0);
   cacheobject.b_miss.set(0);
  }
 ...
 if(saved==0)
 {
   httpreq.close_logfiles(httpreq.logfilez);
   dircache=new HashMap <String, cachedir> (40); // reinit hashtable
...
final public static String[] parseURL(String url,String proto)
{
  ...
  /* najdeme nejvice vlevo z moznych problemovych znaku */
   ...
   /* *************************************************** */
   /* ALERT: Synchronize by hand with cacheobject.genName */
   /* *************************************************** */
...
final static java.util.regex.Pattern[] addRealRegexpToArray(String what,java.util.regex.Pattern[] array)
{
 ...
 catch ( java.util.regex.PatternSyntaxException badsyntax ) {
 ...
 if(array==null) 
 { 
     array=new java.util.regex.Pattern[1];
     array[0]=newrx;
 }
 else {
	 java.util.regex.Pattern[] tmp;
	 tmp=new java.util.regex.Pattern[array.length+1];
	 System.arraycopy(array,0,tmp,0,array.length);
	 tmp[array.length]=newrx;
	 array=tmp;
 }
 return array;
}
...
final static java.util.regex.Pattern[] parseRealRegexpFile(String fname,java.util.regex.Pattern[] oldf)
{
 try
 {
     String line,token;
...
static final public void handleBlock(request req) throws IOException
{
	...
	if(custom403==null) 
		...
		if(custom403.charAt(0)=='0') {
			...
			{
				req.make_headers(200,"image/gif",null,null,request.GIF.length,cacheobject.VINTAGE_DATE,0,"\"Builtin-gif\"");
				req.send_headers();
